...
void OnOuvrir2(GtkWidget* widget, gpointer data)
	{
	GtkWidget *dialog;
	GtkWidget *dialog2;
	GtkFileFilter *pgm_filter;
	GtkFileFilter *pgm_filter2;
	
	pgm_filter = gtk_file_filter_new ();
	gtk_file_filter_set_name( pgm_filter, "*.pgm");
	gtk_file_filter_add_pattern (pgm_filter, "*.pgm");
	pgm_filter2 = gtk_file_filter_new ();
	gtk_file_filter_set_name( pgm_filter2, "*.pgm");
	gtk_file_filter_add_pattern (pgm_filter2, "*.pgm");
	
	dialog = gtk_file_chooser_dialog_new ("Ouvrir le fichier",
				      NULL,
				      GTK_FILE_CHOOSER_ACTION_OPEN,
				      GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
				      GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
	...
	if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)
  	...
  	dialog2 = gtk_file_chooser_dialog_new ("Ouvrir le fichier masqueur",
				      NULL,
				      GTK_FILE_CHOOSER_ACTION_OPEN,
				      GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
				      GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
	...
	gtk_file_chooser_add_filter(GTK_FILE_CHOOSER (dialog2), pgm_filter2);
  	
  	if (gtk_dialog_run (GTK_DIALOG (dialog2)) == GTK_RESPONSE_ACCEPT)
 		{
		
		nom_fichier2 = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (dialog2));
		// effacer historique
		pile_free_images();

		pile_new_image(charg(nom_fichier,nom_fichier2));
